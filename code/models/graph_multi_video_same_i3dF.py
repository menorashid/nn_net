...
class Graph_Multi_Video(nn.Module):
    ...
    def out_f(self, input, epoch_num = None, ret_bg =False, branch_to_test = -1):
        ...
        for input in input_chunks:
            ...
            input_graph = input
            for col_num in range(len(self.graph_layers)):
                input_graph = self.graph_layers[col_num].graph_layer(input_graph, input, to_keep = to_keep)
    ...
    def get_similarity(self,input,idx_graph_layer = 0,sparsify = None, nosum = False):
        ...
        # feature_out = self.linear_layer(input)
        sim_mat = self.graph_layers[idx_graph_layer].get_affinity(input, to_keep = to_keep, nosum = nosum)
